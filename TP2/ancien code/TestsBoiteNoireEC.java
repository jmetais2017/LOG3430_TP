...
class TestsBoiteNoireEC {
	...
	void testEulerianCycle() {
		...
		final int[] NB_VERTICES = {10, 100, 1000};
		final int[] NB_EDGES = {5, 50, 500};
		...
		for(int i=0 ; i< NB_VERTICES.length ; i++)
		{
			...
			// T5 = <{V: valide, K: valide}, {Graphe tel que pour tout V -> adj(V)%2 = 0}>
			...
			// T6 = <{V: valide, K: valide}, {Graphe tel que pour tout V -> somme(adj(V)) = 2*K}>
	...
	void testBipartite() {
		...
		// T4 = <{V1:10, V2:10, E:100}, {Graphe cree sans erreur}>
		try {
		...
		// T5 = <{V1:10, V2:10, E:101}, {erreur}>
		assertThrows(Exception.class, () -> {
			GraphGenerator.bipartite(10, 10, 101);
		});
		
		final int[] NB_VERTICES = {10, 100, 1000};
		final int[] NB_EDGES = {50, 500, 5000};
		...
		for(int i=0 ; i< NB_VERTICES.length ; i++)
		{
			...
			//on s'attend a ce que la somme de chaque noueud possedant une arrete
			//correspond au double du nombre d'arretes definis (lemme des poignees de main)
			// T6 = <{V1: valide, V2: valide, E: valide}, {Graphe tel que pour tout V -> somme(adj(V)) = 2*E}>
			assertEquals(nbEdges*2, sumVerticesEdges);
			...
			while(!queue.isEmpty()) {
				...
				for(int child : g.adj(current)) {
					// T7 = <{V1: valide, V2: valide, E: valide},
	...
	void testRegular() {
		...
		for(int i=0 ; i< NB_VERTICES.length ; i++)
		{
			...
			//T5 = <{V: valide, E: valide}, {Graphe tel que pour tout V -> adj(V) = E}>
			for(int v=0; v<nbVertices; v++)
