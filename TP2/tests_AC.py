...
#SIMPLE
    ...
    def test_simple_raises_value_error_when_vertices_is_same(self):
        for nbVertices in range(-3, -1): # V invalides
            for nbEdges in [-3, -1, 0, 3, 5]: # E valides et invalides (critère AC)
    ...
    def test_simple_raises_value_error_when_edge_is_negative(self):
        for V in [10, 0, -10]: # V valides et invalides  (critère AC)
...
#SIMPLE_WITH_PROBABILITY
    ...
    def test_simple_with_probability_with_invalid_probability(self):
        for V in [10, -10]: # V valide et invalide  (critère AC)
    ...
    def test_simple_with_probability_with_invalid_vertices(self):
        for V in [-3, -2, -1]: # V invalides
            for P in [-0.01, 0.2, 0.5, 0.8, 1, 1.01]: # P valides et invalides  (critère AC)
                #on s'assure qu'une erreur est lancée
                self.assertRaises(ValueError, generators.simple_with_probability, V, P)
...
#BIPARTITE
    ...
    #Test quand V1 est negatif peu importe V2 et E  (critère AC)
    ...
    #Test quand V2 est negatif peu importe V1 et E  (critère AC)
    ...
    #Test quand E est negatif peu importe V1 et V2  (critère AC)
    ...
    #Test quand E est trop gros peu importe V1 et V2  (critère AC)
...
#BIPARTITE_WITH_PROBABILITY
    ...
    #Test quand V1 est negatif peu importe V2 et P  (critère AC)
    ...
    #Test quand V2 est negatif peu importe V1 et P  (critère AC)
    ...
    #Test quand P est invalide peu importe V1 et V2  (critère AC)
...
#REGULAR
    ...
    def test_regular_raises_value_error_when_vertices_is_smaller_than_3(self):
        for nbVertices in range(-3, 3): # V invalides
            for k in [-3, -1, 0, 4, 5]: # E valides et invalides
                #on s'assure qu'une erreur est lancée
    ...
    def test_regular_raises_value_error_when_edge_is_negative(self):
        for V in [10, 0, -10]: # V valides et invalides
            for k in range(-3, -1): # E invalides
                #on s'assure qu'une erreur est lancée
                self.assertRaises(ValueError, generators.regular, V, k)
